...
#include "SpriteRendererComponent.h"  // For checking spriteRenderer->isVisible
#include "Scene.h"                    // For layer visibility checks
#include "Layer.h"                    // For layer visibility checks
...
void SceneWindow::RenderSceneWindow(int newWidth, int newHeight)
{
    ...
    if (selectedEntity && !isPlaying && ImGui::IsMouseDown(ImGuiMouseButton_Left) && !GizmoManager::isDraggingGizmo)
    {
    ...
    if (ImGui::IsMouseReleased(ImGuiMouseButton_Left))
    {
        if (selectedEntity)
        {
            ...
            {
                std::cout << "Object didn't move! so don't save state" << std::endl;
    ...
    if (selectedEntity)
    {
        // Additional debug or highlight logic could go here if needed.
    }
...
{
    ImVec2 fboSize = ImGui::GetContentRegionAvail();
    Vector2D worldScale((CameraManager::GetHeight() * CameraManager::GetAR()) / fboSize.x,
        CameraManager::GetHeight() / fboSize.y);
    return worldScale;
}
...
@brief Retrieves a list of entities at a specific position in the world,
       typically based on the mouse cursor's location. Only returns entities if
       their layer is visible and (if they have a SpriteRenderer) that it's visible.
...
std::vector<Entity*> SceneWindow::GetEntitiesAtPosition(const Vector2D& worldPos)
{
    ...
    for (auto& [entityID, component] : transformComponents)
    {
            ...
            worldPos.y >= top && worldPos.y <= bottom)
        {
            ...
            // Skip entities with the "Background" layer (as per original logic)
            if (entity->layerName == "Background")
            {
                continue;
            }
            ...
            if (activeScene)
            {
                Layer* layer = activeScene->GetLayer(entity->layerName);
            ...
            // If there's a SpriteRenderer, skip if it's not visible
