...
Entity* PlayerLogic::makeObject(ItemType type)
{
	...
	SpriteRendererComponent* spriteRenderer_ = DuckEngine::DUCKENGINE_ComponentManager.AddComponent<SpriteRendererComponent>(newObject->entityID, true);
	spriteRenderer_->sortingOrder = 19;

	spriteRenderer_->texture = AssetManager::GetTextureByName(whatType(type));
