...
void ComponentFactory::SaveComponentsToJson(int entityID, json& componentsArray)
{
	...
	if (auto* sound = DuckEngine::DUCKENGINE_ComponentManager.GetComponent<SoundComponent>(entityID))
	{
		...
		soundData["properties"]["effects"] = sound->effects.first;
		soundData["properties"]["effectValue"] = sound->effects.second;
	...
	if (auto* textComponent = DuckEngine::DUCKENGINE_ComponentManager.GetComponent<TextComponent>(entityID))
	{
		...
		textData["properties"]["stretchWithScreen"] = textComponent->stretchWithScreen;
		...
		textData["properties"]["sortingOrder"] = textComponent->sortingOrder;
...
std::shared_ptr<Component> ComponentFactory::CreateComponentFromJson(const nlohmann::json& componentJson)
{
	...
	else if (type == "SoundComponent")
	{
		...
		std::string effectName = componentJson["properties"].value("effects", "Default");
		float effectValue = componentJson["properties"].value("effectValue", 0.0f);

		auto soundComponent = std::make_shared<SoundComponent>(soundID, category, loop, playOnStart, volume, std::make_pair(effectName, effectValue));
	...
	else if (type == "TextComponent")
	{
		...
		bool stretchWithScreen = componentJson["properties"].value("stretchWithScreen", false);
		...
		textComponent.get()->stretchWithScreen = stretchWithScreen;
